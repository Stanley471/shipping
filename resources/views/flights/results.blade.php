...
@section('title', 'Flight Results | ' . config('app.name'))
    ...
    <div class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <a href="{{ route('flights.search') }}" class="text-blue-600 dark:text-blue-400 text-sm hover:underline mb-2 inline-block">← Back to Search</a>
                ...
                <button onclick="window.location.reload()" class="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors text-sm">
                    Refresh Results
                </button>
            </div>
        </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 hover:shadow-lg transition-shadow">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div class="flex items-center gap-3 min-w-[180px]">
                                <div class="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center text-xl">
                                    ✈️
                                </div>
                                <div>
                            ...
                            <div class="text-right min-w-[120px]">
                                <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">${{ number_format($price) }}</p>
                                ...
                                <form action="{{ route('flights.book') }}" method="GET">
                                    ...
                                    <button type="submit" class="w-full md:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-colors">
                                        Select
                                    </button>
                                </form>
                            </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex flex-wrap gap-4 text-sm text-slate-500 dark:text-slate-400">
                            ...
                            @if(isset($flight['arrival']['baggage']))
                            <span class="flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="text-center py-16">
                ...
                <a href="{{ route('flights.search') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-colors">
                    Modify Search
                </a>
            </div>
        @endif
